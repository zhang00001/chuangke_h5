  ...
  <div class="content">
    ...
    <div class="mian_div">
      ...
      <van-cell-group class="input_group">
        ...
        />
        <van-field v-model="yqm" placeholder="请输入邀请码(必填)" class="ipnt" :error="error_yqm" />
        <!-- :error-message="error_password" -->
      </van-cell-group>
      <!-- <div class="btn_div">登录</div> -->
      <div class="btn_div1" @click="f_login">注册</div>
      <div class="bom_p" style="text-align: right;" @click="$router.push({ name: 'login' })">
        <!-- <span></span>
...
export default {
  ...
  data() {
    return {
      ...
      password: "",
      password2: "",
      yqm: this.$route.params.code == 0 ? "" : this.$route.params.code,
      error_message: "",
      error_pyzm: false,
  ...
  methods: {
    getyzm(wait) {
      if (!this.$common._verPhone(this.iphone)) {
        ...
        this.http
          .post("getCaptcha", { phone: this.iphone, type: 2 })
          .then(res => {
