<template>
  <div class="content">
    ...
    <van-tabs v-model="active" swipeable>
      <van-tab v-for="(itemem,i) in vantab" :key="i" :title="itemem.name">
...
export default {
  ...
  mounted() {
    ...
    this.http.get("categoryList", {}).then(res => {
      console.log(res.data);
      if (res.data.code == 200) {
        this.vantab = res.data.data;
        this.id = this.vantab[0].id;
        // this.http
  ...
  methods: {
    f_tiaozhua() {
      window.location =
        ...
        this.total_List.longitude +
        "," +
        this.total_List.latitude +
        "&title=" +
        this.total_List.address +
        "&content=店铺地址&output=html&src=webapp.baidu.openAPIdemo";
      // "
      // +this.total_List.address +
      // "
    },
